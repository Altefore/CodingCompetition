...
public class Game extends Canvas {
	...
	public boolean gameBeingPlayed = false;
	private boolean lastBoolean = true;
	private boolean cheatCodes = false;
	...
	private HighScoreServerClient scoreServer;
		...
		while (true) {
			...
			if(pause){
				...
				if(cheatCodes==true)
				{
					...
					CheatCode c = new CheatCode(this,code);
				}
			}
	...
	public Game() {
		...
		try {
			scoreServer = new HighScoreServerClient("127.0.0.1");
			maxScore = Integer.parseInt(scoreServer.read());
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	public void gameOver() {
		if(score > maxScore)
		{
			if(scoreServer != null)
			{
				scoreServer.sendData(String.valueOf(score));
			}
			maxScore = score;
		}
	...
	//displays dynamic side bar with high scores and key function discriptions
	...
	// displays player controls and waits for player 1 to spawn
	...
	//displays a custom end of level screen
	...
	//displays a custom start screen with starfield
	public void showSplashScreen() {
		...
		while (!keyHandler.spacePressed) {
			...
			g.setColor(Color.WHITE);
			g.drawString("HIGH SCORE: " + maxScore, getWidth()/3 + 30, getHeight()*3/4+90);
	...
	//displays a custom game over screen
	public void gameOverScreen(Graphics g) {
		...
		g.drawString("HIGH SCORE", width/4+120, height/2+100);
		g.drawString(String.valueOf(maxScore), width/4 +120, height/2+110+35);
		if(score == maxScore)
		{
			g.drawString("CONGRATULATIONS!", width/4+25, height/2+100+45+45);
			g.drawString("NEW HIGH SCORE", width/4+25, height/2+190+45);
		}
	...
	//displays a basic message label with a y offset
	...
	//sets player 1 invincible
	public void set1Invincible(boolean b){
		is1Invincible = b;
	}
	//sets player 2 invincible
